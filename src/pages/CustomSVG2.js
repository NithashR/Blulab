...
const SvgIcon = (props) => {
  const [progress, setProgress] = React.useState(0); // 0..1 where 1 means scrolled 1000px or more
  ...
  React.useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      rafId = requestAnimationFrame(() => {
        const p = Math.min(1, window.scrollY / 1000);
        setProgress(p);
      });
    };

    // set initial progress in case user is already scrolled
    setProgress(Math.min(1, window.scrollY / 1000));
  ...
  // Updated points: now includes the final top-right square so there are 3 segments (3 lines).
  // Make sure each point corresponds to the square center/translate you want.
  const points = [
    { x: 133.19, y: 165.0 }, // start near 'B'
    { x: 165, y: 185 }, // intermediate
    { x: 206, y: 145 }, // intermediate / next square
      ...
      <g id="g10" transform="matrix(1.33333 0 0 -1.33333 0 398.01)">
        <g id="g12">
          <g id="g14" clipPath="url(#clipPath18)">
            ...
            {/* re-added the small light-blue square (g98) as requested earlier */}
            ...
            {/* Connecting lines: reveal the polyline progressively across segments (cumulative).
                End of each segment equals the start of the next (continuous).
                Increased strokeWidth for thicker lines.
            */}
            <g id="connecting-lines" style={{ pointerEvents: "none" }}>
              {segments.map((seg, i) => {
                const accBefore = segments.slice(0, i).reduce((s, ss) => s + ss.len, 0);
                // portion of total length revealed so far
                const revealedLen = progress * totalLen;
                // How much of this segment should be visible [0..1]
                const segProgress = Math.max(0, Math.min(1, (revealedLen - accBefore) / seg.len));
                ...
                return (
                  <line
                    ...
                    y2={seg.q.y}
                    stroke="#2f85c6"
                    strokeWidth={6} // thicker line as requested (adjust value if needed)
                    strokeLinecap="round"
                    ...
                    style={{
                      opacity: 0.98,
                      filter:
                        progress > 0 ? "drop-shadow(0 1px 1px rgba(0,0,0,0.08))" : undefined,
                    }}
                  />
                );
              })}
